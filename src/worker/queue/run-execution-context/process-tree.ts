...
export const isProcessTreeAlive = async (
  _state: RunExecutionContextState,
...
export const waitForProcessTreeToStop = async (
  ...
  while (Date.now() < deadline) {
    if (!(await isProcessTreeAlive(state, session))) {
      try {
...
export const waitForProcessTreeToStopSafely = async (
  ...
  try {
    return await waitForProcessTreeToStop(state, session, timeoutMs);
  } catch (error) {
