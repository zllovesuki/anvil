  ...
  try {
    if (context.state.session) {
      ...
      if (
        ...
        !(await context.runtime.waitForProcessTreeToStopSafely(context.state.session, 5_000, "final_cleanup"))
      ) {
        await context.runtime.hardCancelProcessTree(context.state.session, context.state.currentProcess);
      }
    }
