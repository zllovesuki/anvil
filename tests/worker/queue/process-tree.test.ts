...
} from "@/worker/queue/run-execution-context/process-tree";
import { NO_CONTAINER_INSTANCE_ERROR_SUBSTRING } from "@/worker/sandbox/container-errors";
import type { RunExecutionContextState } from "@/worker/queue/run-execution-context/types";
...
describe("queue process helpers", () => {
  ...
  it("waits for the process tree to stop and cleans completed processes", async () => {
    ...
    await expect(waitForProcessTreeToStop(state, session as never, 50, 0)).resolves.toBe(true);
