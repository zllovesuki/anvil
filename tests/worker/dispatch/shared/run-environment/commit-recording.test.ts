...
describe("run environment preparation", () => {
  describe("resolved commit recording", () => {
    ...
    it("reuses a previously recorded manual-run commit instead of re-resolving branch head", async () => {
      ...
      const { context, events, projectId, recordRunResolvedCommit, runId, sandbox } = createHarness({
        ...
        snapshotCommitSha: null,
        runMetaCommitSha: commitSha,
        execImpl: (command) => {
          if (command.includes(" origin 'main'") || command.includes("checkout -B 'main' FETCH_HEAD")) {
            throw new Error(`Replay should not re-resolve branch head: ${command}`);
          }
          ...
          if (command.includes(`fetch --depth=1 --update-shallow origin '${commitSha}'`)) {
            events.push("fetch-depth-1");
            return createExecResult();
          }
      ...
      expect(events).toContain("checkout-detached");
      expect(events).not.toContain("checkout-branch");
      expect(sandbox.deleteSession).toHaveBeenCalledWith("checkout-session");
      expect(context.state.session).toBeNull();
    });
  });
