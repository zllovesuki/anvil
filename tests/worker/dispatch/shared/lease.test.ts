...
import { CANCEL_GRACE_MS } from "@/worker/dispatch/shared/run-execution-context";
import { RunLease } from "@/worker/dispatch/shared/run-lease";
...
describe("run lease", () => {
  ...
  it("soft-cancels the active process tree when canceling without a live process handle", async () => {
    const context = makeContext();
    context.runtime.getLiveCurrentProcess = vi.fn(() => null);
    context.state.cancelRequestedAt = expectTrusted(UnixTimestampMs, 1_740_000_000_000, "UnixTimestampMs");
    context.state.session = { id: "session-1" } as never;
    ...
    expect(context.logs.appendSystemLog).toHaveBeenCalledWith("Cancellation requested. Stopping active process tree.");
    expect(context.runtime.softCancelProcessTree).toHaveBeenCalledWith(context.state.session, null);
